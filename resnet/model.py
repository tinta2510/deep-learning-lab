...
class ResNet(GenericNet):
    def __init__(self, in_channels, num_classes, filters=[16, 32, 64], 
                 blocks_per_stage=3):
        """
        ResNet implementation using residual blocks.

        Each stage consists of `blocks_per_stage` residual blocks. The first block of each stage
        downsample using stride=2 and a projection shortcut if needed.

        Args:
            in_channels (int): Number of input channels.
            num_classes (int): Number of classification classes.
            filters (list): List of output channels per stage (e.g., [16, 32, 64]).
            blocks_per_stage (int): Number of residual blocks per stage.
        """
...
class PlainBlock(nn.Module):
    def __init__(self, in_channels, out_channels, stride=1, kernel_size=3):
        """
        Simple convolutional block: Conv2d → BatchNorm → ReLU.

        Used in plain (non-residual) networks to build deep stacks of convolutions.
...
class PlainNet(GenericNet):
    def __init__(self, in_channels, num_classes, filters=[16, 32, 64], blocks_per_stage=6):
        ...
        Args:
            ...
            filters (list[int]): List of integers representing the number of filters in each stage.
    ...
    def _make_layers(self, num_filters, stride=1):
        ...
        self.curr_n_filters = num_filters
        for _ in range(1, self.blocks_per_stage):
            layers.append(PlainBlock(num_filters, num_filters))
        return nn.Sequential(*layers)
